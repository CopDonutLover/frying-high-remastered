...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion: # Handle mouse/trackpad movement
		...
		if not held_item == null: # Check if there is currently an item being held
...
func drop_item() -> void:
	if not held_item == null and held_item.is_inside_tree(): # Check if the held_item exists and is in the current scene tree
